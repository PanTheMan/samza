...
class HDFSBackupMetrics(
  ...
  val numFilesBackedUp = newGauge("numFilesBackedUp", 0)
  val totalTimeCreateCheckpoint = newTimer("totalTimeCreateCheckpoint")
  val totalTimeCreateBackup = newTimer("totalTimeCreateBackup");
  val bytesBackedUp = newCounter("bytes-backedup")
  val numFilesDeleted = newGauge("numHDFSFilesDeleted", 0)
  val bytesDeleted = newCounter("hdfsBytesDeleted")
